...
class DetailAnalyzer:
    ...
    def analyze_context_details(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _analyze_energy_indicators(self, content: str) -> Dict[str, Any]:
        ...
        high_energy_words = [
            'energie', 'motiviert', 'aktiv', 'produktiv', 'kraftvoll',
            'lebendig', 'dynamisch', 'begeistert'
        ]
        
        low_energy_words = [
            'müde', 'erschöpft', 'schlapp', 'antriebslos', 'lethargisch',
            'kraftlos', 'ausgelaugt', 'schwermütig'
        ]
    ...
    def _analyze_stress_level(self, content: str) -> Dict[str, Any]:
        ...
        stress_indicators = [
            'stress', 'druck', 'überwältigt', 'angespannt', 'nervös',
            'sorge', 'panik', 'hektik', 'zeitdruck', 'belastet'
        ]
        
        relief_indicators = [
            'entspannt', 'ruhig', 'gelassen', 'friedlich', 'ausgeglichen',
            'erleichtert', 'befreit', 'locker'
        ]
    ...
    def _classify_context_type(self, content: str, tags: List[str]) -> Dict[str, Any]:
        ...
        tag_scores = {}
        for category, keywords in context_categories.items():
            score = sum(1 for tag in tags for keyword in keywords if keyword in tag.lower())
            tag_scores[category] = score

        # Kombiniere Scores
        combined_scores = {}
        for category in context_categories.keys():
            combined_scores[category] = content_scores[category] + tag_scores[category] * 2  # Tags gewichten mehr
    ...
    def _assess_actionability(self, content: str) -> Dict[str, Any]:
        ...
        # Action-Indikatoren
        action_words = [
            'machen', 'tun', 'starten', 'beginnen', 'planen', 'umsetzen',
            'ändern', 'verbessern', 'entwickeln', 'arbeiten', 'lernen'
        ]
        
        # Problem-Indikatoren
        problem_words = [
            'problem', 'schwierigkeit', 'hindernis', 'herausforderung',
            'blockiert', 'festgefahren', 'unsicher'
        ]
        
        # Reflexion-Indikatoren
        reflection_words = [
            'denke', 'fühle', 'merke', 'erkenne', 'verstehe',
            'reflektiere', 'überlege', 'bewusst'
        ]
        ...
        return {
            ...
            'recommendation': self._get_actionability_recommendation(actionability, primary_mode)
        }
    ...
    def _assess_complexity(self, content: str) -> Dict[str, Any]:
        ...
        complexity_indicators = [
            'komplex', 'kompliziert', 'schwierig', 'vielschichtig',
            'mehrere', 'verschiedene', 'sowohl', 'einerseits', 'andererseits'
        ]
        
        # Einfachheits-Indikatoren
        simplicity_indicators = [
            'einfach', 'klar', 'eindeutig', 'simpel', 'direkt', 'schnell'
        ]

        content_lower = content.lower()
        complexity_count = sum(1 for indicator in complexity_indicators if indicator in content_lower)
        simplicity_count = sum(1 for indicator in simplicity_indicators if indicator in content_lower)
    ...
    def _detect_personal_patterns(self, content: str, tags: List[str]) -> Dict[str, Any]:
        """
    ...
    def _calculate_analysis_confidence(self, analysis: Dict[str, Any]) -> float:
        ...
        if analysis['emotional_state']['indicators_found'] > 0:
            confidence_factors.append(analysis['emotional_state']['confidence'])
        
        # Urgency Konfidenz
        urgency_confidence = min(analysis['urgency_level']['score'] / 3, 1.0)
        confidence_factors.append(urgency_confidence)
        
        # Kontext-Typ Konfidenz
        context_confidence = min(analysis['context_type']['confidence'] / 3, 1.0)
        confidence_factors.append(context_confidence)
        
        # Actionability Konfidenz
        actionability_indicators = (
            analysis['actionability']['action_indicators'] +
            analysis['actionability']['problem_indicators'] +
            analysis['actionability']['reflection_indicators']
        )
        actionability_confidence = min(actionability_indicators / 5, 1.0)
        confidence_factors.append(actionability_confidence)
    ...
    def _load_emotion_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'positive': [
                'gut', 'toll', 'super', 'fantastisch', 'glücklich', 'zufrieden',
                'erfolgreich', 'stolz', 'freude', 'begeistert', 'dankbar'
            ],
            'negative': [
                'schlecht', 'mies', 'furchtbar', 'traurig', 'deprimiert',
                'frustriert', 'ärgerlich', 'enttäuscht', 'sorge', 'angst'
            ],
            'energetic': [
                'energie', 'dynamisch', 'aktiv', 'lebhaft', 'motiviert',
                'kraftvoll', 'lebendig', 'antrieb'
            ],
            'calm': [
                'ruhig', 'entspannt', 'gelassen', 'friedlich', 'ausgeglichen',
                'besonnen', 'still'
            ],
            'stressed': [
                'gestresst', 'überwältigt', 'unter druck', 'angespannt',
                'nervös', 'hektisch', 'chaotisch'
            ]
        }
    ...
    def _load_urgency_markers(self) -> List[str]:
        ...
        return [
            'dringend', 'sofort', 'schnell', 'eilig', 'deadline',
            'frist', 'heute noch', 'asap', 'wichtig', 'kritisch',
            'problem', 'krise', 'notfall'
        ]
    ...
    def _calculate_valence(self, emotions: Dict[str, float]) -> str:
        ...
        positive_score = emotions.get('positive', 0) + emotions.get('energetic', 0) + emotions.get('calm', 0)
    ...
    def _get_optimal_activities(self, time_period: str, day_type: str) -> List[str]:
        ...
        activities = {
            ...
            'afternoon': {
                'weekday': ['Meetings', 'Collaboration', 'Implementation', 'Communication'],
                'weekend': ['Social Activities', 'Errands', 'Projects', 'Exploration']
            },
            'evening': {
                'weekday': ['Reflection', 'Planning Tomorrow', 'Relaxation', 'Family Time'],
                'weekend': ['Entertainment', 'Social Time', 'Hobbies', 'Reflection']
            },
            'night': {
                'weekday': ['Rest', 'Light Reading', 'Meditation', 'Preparation for Sleep'],
